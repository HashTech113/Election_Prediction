...
export function TamilNaduApp() {
  ...
  // Once the Default tab data has landed, warm the cache for every analysis
  // tab in the background so the first click on each tab is instant. The
  // fetcher in api.ts dedupes inflight requests and persists the result, so
  // these calls cost nothing if the tab has already been visited.
  useEffect(() => {
    ...
    idle(() => {
      for (const t of ANALYSIS_TYPES) {
        // Errors here are silent — the AnalysisPanel will surface them on
        // demand if the user actually clicks the tab.
        void fetchAnalysisPredictions(t).catch(() => undefined);
      }
    });
  ...
  return (
    <div className="tamilnadu-app app-shell">
