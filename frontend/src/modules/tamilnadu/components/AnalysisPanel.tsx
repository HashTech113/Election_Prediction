...
import { fetchAnalysisPredictions, getCachedAnalysisPredictions } from "../services/api";
import {
  AnalysisMeta,
...
export function AnalysisPanel({ analysisType }: AnalysisPanelProps) {
  // Hydrate synchronously from the in-memory cache so previously-visited tabs
  // render instantly with no loading flash on switch.
  const [data, setData] = useState<AnalysisResponse | null>(
    () => getCachedAnalysisPredictions(analysisType) ?? null,
  );
  const [loading, setLoading] = useState(
    () => !getCachedAnalysisPredictions(analysisType),
  );
  ...
  useEffect(() => {
    ...
    if (cached) {
      // Fast path: instant tab switch, no fetch, no loading state.
      setData(cached);
      setLoading(false);
      setError(null);
      return;
    ...
    setError(null);
    // Note: we deliberately do NOT clear `data` here. Keeping the previous
    // tab's render on screen while the next tab loads removes the jarring
    // "Loading..." flash. We still flip `loading=true` so the inline pill
  ...
  // Only render once the data we have actually belongs to the current tab.
  // Pairing labels (which use the prop) with stale rows (from a previous tab)
  // would be misleading. Repeat visits are instant via the in-memory cache.
  if (!data || data.meta.analysis_type !== analysisType) {
    if (loading) {
