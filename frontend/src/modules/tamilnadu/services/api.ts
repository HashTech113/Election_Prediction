...
function inferApiBaseFromWindow(): string {
  ...
  if (isPrivateIpv4(hostname)) {
    const inferredPort = String(import.meta.env.VITE_API_PORT || "8002").trim();
...
// Per-state env var (preferred) takes precedence; legacy single-backend
// vars are retained as a fallback for older deployments.
const EXPLICIT_API_BASE =
  normalizeApiBase(import.meta.env.VITE_API_TN_URL) ||
...
// ---- Static data mode -----------------------------------------------------
// When enabled, the app reads pre-baked JSON snapshots from `public/data/tamilnadu`
...
// the local backend. Force either way with VITE_STATIC_DATA="1" / "0".
...
const STATIC_DATA_BASE = `${import.meta.env.BASE_URL}data/tamilnadu`;
...
function resolveStaticUrl(path: string): string {
  ...
  } else if (rawPath.startsWith("/api/predictions")) {
    const analysisType = query.get("analysis_type");
    file = analysisType ? `analysis-${analysisType}.json` : "predictions.json";
  }
...
// Default per-request timeout. Railway free-tier services cold-boot in
// ~5–8s, so 10s is a generous ceiling that still surfaces real outages
...
function withTimeout(
  ...
  if (userSignal?.aborted) return init;
...
async function fetchWithApiFallback(
  ...
  try {
    const response = await fetch(primaryUrl, withTimeout(init));
    if (
      fallbackUrl &&
      (response.status === 404 ||
        response.status === 502 ||
      ...
      return fetch(fallbackUrl, withTimeout(init));
    }
    return response;
  } catch (error) {
...
// Per-tab in-memory cache. Once a tab's data has been fetched in this session,
// subsequent switches resolve synchronously and avoid re-hitting the backend,
// which is what made tab switching feel slow.
const analysisResponseCache = new Map<AnalysisType, AnalysisResponse>();
const analysisInflight = new Map<AnalysisType, Promise<AnalysisResponse>>();

export function getCachedAnalysisPredictions(
  analysisType: AnalysisType
): AnalysisResponse | undefined {
  return analysisResponseCache.get(analysisType);
}
...
export async function fetchAnalysisPredictions(
  ...
  const cached = analysisResponseCache.get(analysisType);
  if (cached) return cached;

  const inflight = analysisInflight.get(analysisType);
  ...
  const request = (async () => {
    ...
    analysisResponseCache.set(analysisType, body);
    return body;
  })();

  analysisInflight.set(analysisType, request);
  try {
    return await request;
  } finally {
    analysisInflight.delete(analysisType);
  }
}
